...
SUBROUTINE COMPUTE_FUNCTION_THERMO (D, PALP, PBETA, PGAM, PLTT, PC, PT, PEXN, PCP, PLOCPEXN, PAMOIST, PATHETA, PRT, &
  & PDRVSATDT, PRVSAT, CST, PPABST, KRR)
    !     ########################################################################
    !!
    ...
    !*       1.1 Lv/Cph at  t
    !
    ! present(PRVSAT,PDRVSATDT) ! present(PLOCPEXN) ! present(PDRVSATDT)
...
!$acc end kernels
    !
!$acc kernels present_cr( PRVSAT,PDRVSATDT )
!$mnh_expand_array ( JIJ=IIJB:IIJE,JK=1:IKT )
    ...
    PRVSAT(IIJB:IIJE, 1:IKT) = PRVSAT(IIJB:IIJE, 1:IKT)*ZEPS / MAX(1.E-3, PPABST(IIJB:IIJE, 1:IKT) - PRVSAT(IIJB:IIJE, 1:IKT))
    !
    ...
    !*      1.5 compute Amoist
    !
    PAMOIST(IIJB:IIJE, 1:IKT) = 0.5 / (1.0 + PDRVSATDT(IIJB:IIJE, 1:IKT)*PLOCPEXN(IIJB:IIJE, 1:IKT))
!$mnh_end_expand_array ( JIJ=IIJB:IIJE,JK=1:IKT )
!$acc end kernels
    !
    !*      1.6 compute Atheta
...
!$acc kernels
!$mnh_expand_array ( JIJ=IIJB:IIJE,JK=1:IKT )
    PATHETA(IIJB:IIJE, 1:IKT) = PAMOIST(IIJB:IIJE, 1:IKT)*PEXN(IIJB:IIJE, 1:IKT)*((PRVSAT(IIJB:IIJE, 1:IKT) - PRT(IIJB:IIJE,  &
    & 1:IKT, 1))*PLOCPEXN(IIJB:IIJE, 1:IKT) / (1. + PDRVSATDT(IIJB:IIJE, 1:IKT)*PLOCPEXN(IIJB:IIJE, 1:IKT))*(PRVSAT(IIJB:IIJE,  &
    & 1:IKT)*(1. + PRVSAT(IIJB:IIJE, 1:IKT) / ZEPS)*(-2.*PBETA / PT(IIJB:IIJE, 1:IKT) + PGAM) / PT(IIJB:IIJE, 1:IKT)**2 +  &
    & PDRVSATDT(IIJB:IIJE, 1:IKT)*(1. + 2.*PRVSAT(IIJB:IIJE, 1:IKT) / ZEPS)*(PBETA / PT(IIJB:IIJE, 1:IKT) - PGAM) / PT(IIJB:IIJE, &
    &  1:IKT)) - PDRVSATDT(IIJB:IIJE, 1:IKT))
!$mnh_end_expand_array ( JIJ=IIJB:IIJE,JK=1:IKT )
!$acc end kernels
    !
