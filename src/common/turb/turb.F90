SUBROUTINE TURB (CST, CSTURB, BUCONF, TURBN, NEBN, D, TLES, KRR, KRRL, KRRI, HLBCX, HLBCY, KGRADIENTSLEO, KGRADIENTSGOG, KHALO,  &
& KSPLIT, OCLOUDMODIFLM, KSV, KSV_LGBEG, KSV_LGEND, KSV_LIMA_NR, KSV_LIMA_NS, KSV_LIMA_NG, KSV_LIMA_NH, O2D, ONOMIXLG, OFLAT,  &
& OCOUPLES, OBLOWSNOW, OIBM, OFLYER, OCOMPUTE_SRC, PRSNOW, OOCEAN, ODEEPOC, ODIAG_IN_RUN, HTURBLEN_CL, HCLOUD, HELEC, PTSTEP,  &
& TPFILE, PDXX, PDYY, PDZZ, PDZX, PDZY, PZZ, PDIRCOSXW, PDIRCOSYW, PDIRCOSZW, PCOSSLOPE, PSINSLOPE, PRHODJ, PTHVREF, PHGRADLEO,  &
& PHGRADGOG, PZS, PSFTH, PSFRV, PSFSV, PSFU, PSFV, PPABST, PUT, PVT, PWT, PTKET, PSVT, PSRCT, PLENGTHM, PLENGTHH, MFMOIST,  &
& PBL_DEPTH, PSBL_DEPTH, PCEI, PCEI_MIN, PCEI_MAX, PCOEF_AMPL_SAT, PTHLT, PRT, PRUS, PRVS, PRWS, PRTHLS, PRRS, PRSVS, PRTKES,  &
& PSIGS, PFLXZTHVMF, PFLXZUMF, PFLXZVMF, PWTH, PWRC, PWSV, PDP, PTP, PTDIFF, PTDISS, TBUDGETS, KBUDGETS, PEDR, PLEM, PRTKEMS,  &
& PDPMF, PTPMF, PDRUS_TURB, PDRVS_TURB, PDRTHLS_TURB, PDRRTS_TURB, PDRSVS_TURB, PTR, PDISS, PIBM_LS, PIBM_XMUT,  &
& PCURRENT_TKE_DISS, PSSTFL, PSSTFL_C, PSSRFL_C, PSSUFL_C, PSSVFL_C, PSSUFL, PSSVFL)
  !     #################################################################
  !
  !
  ...
  !
  USE MODD_BUDGET, ONLY: NBUDGET_U, NBUDGET_V, NBUDGET_W, NBUDGET_TH, NBUDGET_RV, NBUDGET_RC, NBUDGET_RI, NBUDGET_SV1,  &
  & NBUDGET_RG, NBUDGET_RH, NBUDGET_RR, NBUDGET_RS, TBUDGETDATA, TBUDGETCONF_t
...
!$mnh_undef ( OPENACC )
  ...
  INTEGER, INTENT(IN) :: KSV_LGBEG  ! number of scalar variables
  INTEGER, INTENT(IN) :: KSV_LGEND  ! number of scalar variables
  INTEGER, INTENT(IN) :: KSV_LIMA_NR
  INTEGER, INTENT(IN) :: KSV_LIMA_NS
  INTEGER, INTENT(IN) :: KSV_LIMA_NG
  INTEGER, INTENT(IN) :: KSV_LIMA_NH
  CHARACTER(LEN=4), INTENT(IN), DIMENSION(2) :: HLBCX  ! X- and Y-direc LBC
  CHARACTER(LEN=4), INTENT(IN), DIMENSION(2) :: HLBCY  ! X- and Y-direc LBC
  ...
  !
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PDXX
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PDYY
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PDZZ
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PDZX
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PDZY
  ! metric coefficients
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PZZ  !  physical distance
  ! between 2 succesive grid points along the K direction
  REAL, INTENT(IN), DIMENSION(D%NIJT) :: PDIRCOSXW
  REAL, INTENT(IN), DIMENSION(D%NIJT) :: PDIRCOSYW
  REAL, INTENT(IN), DIMENSION(D%NIJT) :: PDIRCOSZW
  ! Director Cosinus along x, y and z directions at surface w-point
  REAL, INTENT(IN), DIMENSION(D%NIJT) :: PCOSSLOPE  ! cosinus of the angle
  ! between i and the slope vector
  REAL, INTENT(IN), DIMENSION(D%NIJT) :: PSINSLOPE  ! sinus of the angle
  ! between i and the slope vector
  REAL, INTENT(IN), DIMENSION(D%NIJT) :: PZS  ! orography (for LEONARD terms)
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PRHODJ  ! dry density * Grid size
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: MFMOIST  ! moist mass flux dual scheme
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PTHVREF  ! Virtual Potential
  ! Temperature of the reference state
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT, KGRADIENTSLEO) :: PHGRADLEO  ! horizontal gradients in Moeng
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT, KGRADIENTSGOG) :: PHGRADGOG  ! horizontal gradients in Goger
  !
  REAL, INTENT(IN), DIMENSION(D%NIJT) :: PSFTH
  REAL, INTENT(IN), DIMENSION(D%NIJT) :: PSFRV
  REAL, INTENT(IN), DIMENSION(D%NIJT) :: PSFU
  REAL, INTENT(IN), DIMENSION(D%NIJT) :: PSFV
  ! normal surface fluxes of theta and Rv
  ! normal surface fluxes of (u,v) parallel to the orography
  REAL, INTENT(IN), DIMENSION(D%NIJT, KSV) :: PSFSV
  ! normal surface fluxes of Scalar var.
  !
  !    prognostic variables at t- deltat
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PPABST  ! Pressure at time t
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PUT  ! wind components
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PVT  ! wind components
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PWT  ! wind components
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PTKET  ! TKE
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT, KSV) :: PSVT  ! passive scal. var.
  REAL, INTENT(IN), DIMENSION(MERGE(D%NIJT, 0, OCOMPUTE_SRC), MERGE(D%NKT, 0, OCOMPUTE_SRC)) :: PSRCT  ! Second-order flux
  ! s'rc'/2Sigma_s2 at time t-1 multiplied by Lambda_3
  REAL, INTENT(INOUT), DIMENSION(D%NIJT) :: PBL_DEPTH  ! BL height for TOMS
  REAL, INTENT(INOUT), DIMENSION(D%NIJT) :: PSBL_DEPTH  ! SBL depth for RMC01
  !
  !    variables for cloud mixing length
  REAL, INTENT(IN), DIMENSION(MERGE(D%NIJT, 0, OCLOUDMODIFLM), MERGE(D%NKT, 0, OCLOUDMODIFLM)) :: PCEI
  ...
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: PTHLT  ! conservative pot. temp.
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT, KRR) :: PRT  ! water var.  where
  ! PRT(:,:,:,1) is the conservative mixing ratio
  !
  ! sources of momentum, conservative potential temperature, Turb. Kin. Energy,
  ! TKE dissipation
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: PRUS
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: PRVS
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: PRWS
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: PRTHLS
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: PRTKES
  ! Source terms for all water kinds, PRRS(:,:,:,1) is used for the conservative
  ! mixing ratio
  REAL, OPTIONAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PRTKEMS
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT, KRR) :: PRRS
  ! Source terms for all passive scalar variables
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT, KSV) :: PRSVS
  ! Sigma_s at time t+1 : square root of the variance of the deviation to the
  ! saturation
  REAL, INTENT(OUT), DIMENSION(MERGE(D%NIJT, 0, OCOMPUTE_SRC), MERGE(D%NKT, 0, OCOMPUTE_SRC)) :: PSIGS
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PDRUS_TURB  ! evolution of rhoJ*U   by turbulence only
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PDRVS_TURB  ! evolution of rhoJ*V   by turbulence only
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PDRTHLS_TURB  ! evolution of rhoJ*thl by turbulence only
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PDRRTS_TURB  ! evolution of rhoJ*rt  by turbulence only
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT, KSV) :: PDRSVS_TURB  ! evolution of rhoJ*Sv  by turbulence only
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PFLXZTHVMF
  !                                           MF contribution for vert. turb. transport
  !                                           used in the buoy. prod. of TKE
  REAL, INTENT(OUT), DIMENSION(MERGE(D%NIJT, 0, OFLYER), MERGE(D%NKT, 0, OFLYER)) :: PWTH  ! heat flux
  REAL, INTENT(OUT), DIMENSION(MERGE(D%NIJT, 0, OFLYER), MERGE(D%NKT, 0, OFLYER)) :: PWRC  ! cloud water flux
  REAL, INTENT(OUT), DIMENSION(MERGE(D%NIJT, 0, OFLYER), MERGE(D%NKT, 0, OFLYER), MERGE(KSV, 0, OFLYER)) :: PWSV  ! scalar flux
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PFLXZUMF  ! MF contribution for vert. turb. transport (dyn prod)
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PFLXZVMF  ! MF contribution for vert. turb. transport (dyn prod)
  REAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PTP  ! Thermal TKE production MassFlux + turb
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PTPMF  ! Thermal TKE production MassFlux Only
  REAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PDP  ! Dynamic TKE production MassFlux + turb
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PDPMF  ! Dynamic TKE production MassFlux Only
  REAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PTDIFF  ! Diffusion TKE term
  REAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PTDISS  ! Dissipation TKE term
  !
  TYPE(TBUDGETDATA), INTENT(INOUT), DIMENSION(KBUDGETS) :: TBUDGETS
  ...
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PLENGTHM
  REAL, INTENT(IN), DIMENSION(D%NIJT, D%NKT) :: PLENGTHH
  !
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PEDR  ! EDR
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PLEM  ! Mixing length
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PTR  ! Transport prod. of TKE
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(D%NIJT, D%NKT) :: PDISS  ! Dissipation of TKE
  REAL, OPTIONAL, INTENT(INOUT), DIMENSION(MERGE(D%NIJT, 0, ODIAG_IN_RUN), MERGE(D%NKT, 0, ODIAG_IN_RUN)) :: PCURRENT_TKE_DISS  ! if ODIAG_IN_RUN in mesonh
  REAL, OPTIONAL, INTENT(IN), DIMENSION(MERGE(D%NIJT, 0, OCOUPLES)) :: PSSTFL  ! Time evol Flux of T at sea surface (LOCEAN)
  REAL, OPTIONAL, INTENT(IN), DIMENSION(MERGE(D%NIJT, 0, OCOUPLES)) :: PSSTFL_C  ! O-A interface flux for theta(LOCEAN and LCOUPLES)
  REAL, OPTIONAL, INTENT(IN), DIMENSION(MERGE(D%NIJT, 0, OCOUPLES)) :: PSSRFL_C  ! O-A interface flux for vapor (LOCEAN and LCOUPLES)
  REAL, OPTIONAL, INTENT(IN), DIMENSION(MERGE(D%NIJT, 0, OCOUPLES)) :: PSSUFL_C  ! Time evol Flux of U at sea surface (LOCEAN)
  REAL, OPTIONAL, INTENT(IN), DIMENSION(MERGE(D%NIJT, 0, OCOUPLES)) :: PSSVFL_C  !
  REAL, OPTIONAL, INTENT(IN), DIMENSION(MERGE(D%NIJT, 0, OCOUPLES)) :: PSSUFL
  REAL, OPTIONAL, INTENT(IN), DIMENSION(MERGE(D%NIJT, 0, OCOUPLES)) :: PSSVFL  !
  !
  REAL, OPTIONAL, INTENT(OUT), DIMENSION(MERGE(D%NIJT, 0, OIBM), MERGE(D%NKT, 0, OIBM)) :: PIBM_XMUT  ! IBM turbulent viscosity
  REAL, OPTIONAL, INTENT(IN), DIMENSION(MERGE(D%NIJT, 0, OIBM), MERGE(D%NKT, 0, OIBM)) :: PIBM_LS  ! IBM Level-set function
  !
  !
  !-------------------------------------------------------------------------------
  !
  ...
  !
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZCP
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZEXN
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZT
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZLOCPEXNM
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZLM
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZLMW
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZLEPS
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZTRH
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZATHETA
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZAMOIST
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZCOEF_DISS
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZFRAC_ICE
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZMWTH
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZMWR
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZMTH2
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZMR2
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZMTHR
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZFWTH
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZFWR
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZFTH2
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZFR2
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZFTHR
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZTHLM
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZRTKEMS
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZSHEAR
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZDUDZ
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZDVDZ
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZLVOCPEXNM
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZLSOCPEXNM
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZATHETA_ICE
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZAMOIST_ICE
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZRVSAT
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZDRVSATDT
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZWORK1
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZWORK2
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZETHETA
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZEMOIST
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZDTHLDZ
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZDRTDZ
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZCOEF_AMPL
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT) :: ZLM_CLOUD
  REAL, DIMENSION(D%NIJT, D%NKT) :: ZTEMP_BUD
  ! Cp at t-1
  ! EXN at t-1
  ! T at t-1
  ! Lv/Cp/EXNREF at t-1
  ! Turbulent mixing length (+ work array)
  ! Dissipative length
  !
  ! coefficients for s = f (Thetal,Rnp)
  ! 1/(Cph*Exner) for dissipative heating
  ! ri fraction of rc+ri
  ! 3rd order moments
  ! opposite of verticale derivate of 3rd order moments
  ! initial potential temp; TKE advective source
  ! horizontal-wind vertical gradient
  ! Lv/Cp/EXNREF and Ls/Cp/EXNREF at t-1
  ! coefficients for s = f (Thetal,Rnp)
  ! local array for routine compute_function_thermo
  ! working array syntax
  ! coef ETHETA and EMOIST (for DEAR routine)
  ! dtheta_l/dz, drt_dz used for computing the stablity criterion
  ! Amplification coefficient of the mixing length
  ! when the instability criterium is verified (routine CLOUD_MODIF_LM)
  ! Turbulent mixing length in the clouds (routine CLOUD_MODIF_LM)
  !
  !
  REAL, INTENT(INOUT), DIMENSION(D%NIJT, D%NKT, KRR) :: ZRM  ! initial mixing ratio
  REAL, DIMENSION(D%NIJT) :: ZTAU11M
  REAL, DIMENSION(D%NIJT) :: ZTAU12M
  REAL, DIMENSION(D%NIJT) :: ZTAU22M
  REAL, DIMENSION(D%NIJT) :: ZTAU33M
  REAL, DIMENSION(D%NIJT) :: ZUSLOPE
  REAL, DIMENSION(D%NIJT) :: ZVSLOPE
  REAL, DIMENSION(D%NIJT) :: ZCDUEFF
  REAL, DIMENSION(D%NIJT) :: ZUSTAR
  REAL, DIMENSION(D%NIJT) :: ZLMO
  REAL, DIMENSION(D%NIJT) :: ZRVM
  REAL, DIMENSION(D%NIJT) :: ZSFRV
  REAL, INTENT(INOUT), DIMENSION(D%NIJT) :: ZWORK2D
  ...
  REAL :: ZRVORD  ! RV/RD
  REAL, INTENT(INOUT) :: ZEPS  ! XMV / XMD
  REAL, INTENT(INOUT) :: ZD  ! distance to the surface (for routine DELT)
  REAL, INTENT(INOUT) :: ZVAR  ! Intermediary variable (for routine DEAR)
  REAL, INTENT(INOUT) :: ZPENTE  ! Slope of the amplification straight line (for routine CLOUD_MODIF_LM)
  REAL, INTENT(INOUT) :: ZCOEF_AMPL_CEI_NUL  ! Ordonnate at the origin of the
  ! amplification straight line (for routine CLOUD_MODIF_LM)
  !
  INTEGER, INTENT(INOUT) :: IIJB  ! index value for the
  INTEGER, INTENT(INOUT) :: IIJE  ! index value for the
  INTEGER, INTENT(INOUT) :: IKB  ! index value for the
  INTEGER, INTENT(INOUT) :: IKE  ! index value for the
  ! Beginning and the End of the physical domain for the mass points
  INTEGER, INTENT(INOUT) :: IKT  ! array size in k direction
  INTEGER, INTENT(INOUT) :: IKA  ! array size in k direction
  INTEGER, INTENT(INOUT) :: IKU  ! array size in k direction
  INTEGER, INTENT(INOUT) :: IKL
  INTEGER, INTENT(INOUT) :: IKTB  ! start, end of k loops in physical domain
  INTEGER, INTENT(INOUT) :: IKTE  ! start, end of k loops in physical domain
  INTEGER :: JRR  ! loop counters
  INTEGER, INTENT(INOUT) :: JK  ! loop counters
  INTEGER :: JSV  ! loop counters
  INTEGER, INTENT(INOUT) :: JIJ  ! loop counters
  REAL :: ZL0  ! Max. Mixing Length in Blakadar formula
  REAL, INTENT(INOUT) :: ZALPHA  ! work coefficient :
  ! - proportionnality constant between Dz/2 and
  !                                   !   BL89 mixing length near the surface
  !
  REAL :: ZTIME1
  REAL :: ZTIME2
  LOGICAL, INTENT(INOUT) :: GOCEAN  !Intermediate variable used to work around a Cray compiler bug (CCE 13.0.0)
  LOGICAL :: GTURBLEN_BL89_TURBLEN_RM17_TURBLEN_HM21_ORMC01
  TYPE(TFIELDMETADATA), INTENT(INOUT) :: TZFIELD
  !
  REAL, DIMENSION(D%NIJT, D%NKT, MERGE(KSV + KRR, KSV, TURBN%LTURB_PRECIP)) :: ZWORKT
  REAL, DIMENSION(D%NIJT, D%NKT, MERGE(KSV + KRR, KSV, TURBN%LTURB_PRECIP)) :: ZWORKS
...
!$acc end kernels
    ...
    IF (KRRI >= 1) THEN
      IF (NEBN%LSTATNW) THEN
        ...
        CALL COMPUTE_FUNCTION_THERMO_NEW_STAT(D, PALP, PBETA, PGAM, PLTT, PC, PT(D%NIJT, D%NKT), PEXN(D%NIJT, D%NKT),  &
        & PCP(D%NIJT, D%NKT), PLOCPEXN(D%NIJT, D%NKT), PAMOIST(D%NIJT, D%NKT), PATHETA(D%NIJT, D%NKT), IIJB, IKT,  &
        & ZDRVSATDT(IIJB:IIJE, 1:IKT), ZHOOK_HANDLE2, ZEPS, IIJE, ZRVSAT(IIJB:IIJE, 1:IKT), CST, PPABST(IIJB:IIJE, 1:IKT))
        CALL COMPUTE_FUNCTION_THERMO_NEW_STAT(D, PALP, PBETA, PGAM, PLTT, PC, PT(D%NIJT, D%NKT), PEXN(D%NIJT, D%NKT),  &
        & PCP(D%NIJT, D%NKT), PLOCPEXN(D%NIJT, D%NKT), PAMOIST(D%NIJT, D%NKT), PATHETA(D%NIJT, D%NKT), IIJB, IKT,  &
        & ZDRVSATDT(IIJB:IIJE, 1:IKT), ZHOOK_HANDLE2, ZEPS, IIJE, ZRVSAT(IIJB:IIJE, 1:IKT), CST, PPABST(IIJB:IIJE, 1:IKT))
      ELSE
        CALL COMPUTE_FUNCTION_THERMO(D, PALP, PBETA, PGAM, PLTT, PC, PT(D%NIJT, D%NKT), PEXN(D%NIJT, D%NKT), PCP(D%NIJT, D%NKT),  &
        & PLOCPEXN(D%NIJT, D%NKT), PAMOIST(D%NIJT, D%NKT), PATHETA(D%NIJT, D%NKT), IIJB, PRT(IIJB:IIJE, 1:IKT, 1), IKT,  &
        & ZDRVSATDT(IIJB:IIJE, 1:IKT), ZHOOK_HANDLE2, ZEPS, IIJE, ZRVSAT(IIJB:IIJE, 1:IKT), CST, PPABST(IIJB:IIJE, 1:IKT))
        CALL COMPUTE_FUNCTION_THERMO(D, PALP, PBETA, PGAM, PLTT, PC, PT(D%NIJT, D%NKT), PEXN(D%NIJT, D%NKT), PCP(D%NIJT, D%NKT),  &
        & PLOCPEXN(D%NIJT, D%NKT), PAMOIST(D%NIJT, D%NKT), PATHETA(D%NIJT, D%NKT), IIJB, PRT(IIJB:IIJE, 1:IKT, 1), IKT,  &
        & ZDRVSATDT(IIJB:IIJE, 1:IKT), ZHOOK_HANDLE2, ZEPS, IIJE, ZRVSAT(IIJB:IIJE, 1:IKT), CST, PPABST(IIJB:IIJE, 1:IKT))
...
!$acc end kernels
      ...
      & IKU, PDIRCOSZW(JIJ), IKTB, ZHOOK_HANDLE2, IIJE, ZD, GOCEAN, IKL, IKB, TURBN, JK, PDXX, IKTE, ODZ=.false.)
...
!$acc end kernels
    ...
    !           -----------------------
    IF (OCLOUDMODIFLM) CALL CLOUD_MODIF_LM(ZLM_CLOUD(:, :), D, TPFILE, PRT(JIJ, JK + IKL, 1), PTKET, PDZZ(JIJ, JK + IKL),  &
    & TZFIELD,   ZLM(IIJB:IIJE, 1:IKT), PZZ(JIJ, IKTE + 1), ZETHETA(JIJ, JK), ZSHEAR(:, :), KRRI, IIJE, ZD, ZEMOIST(IIJB:IIJE,  &
    & IKB), CST,   CSTURB, ZCOEF_AMPL(IIJB:IIJE, 1:IKT), PTHVREF(JIJ, JK), ZDRTDZ(IIJB:IIJE, IKB), ZDTHLDZ(JIJ, JK), OOCEAN, IKT, &
    &  IKU, ZPENTE,   PTHLT(JIJ, JK + IKL), ZWORK2(IIJB:IIJE, 1:IKT), GOCEAN, ZTHLM, ZRM, JK, IKTE, ZLOCPEXNM, OCOMPUTE_SRC,  &
    & PSRCT, PCOEF_AMPL_SAT,   IKE, JIJ, ZAMOIST, IKA, ZCOEF_AMPL_CEI_NUL, ZALPHA, PDIRCOSZW(JIJ), ZWORK1(IIJB:IIJE, 1:IKT),  &
    & IKTB, PCEI(IIJB:IIJE, 1:IKT),   IKL, TURBN, PCEI_MIN, PDXX, ZVAR, IIJB, O2D, HTURBLEN_CL, PDYY, ZHOOK_HANDLE2, KRR,  &
    & ZWORK2D(IIJB:IIJE), IKB, PCEI_MAX, ZATHETA)
  END IF
  ! end LHARRAT
